...
{
    size_t num_samples;  // Número de muestras cargadas
    int error_code;      // Variable para almacenar el código de error

    // Ruta del archivo de muestras
    char file_sample_str[60];
    sprintf(file_sample_str, "backend/Core/Samples/0");

    // Cargar los datos con la nueva función cargar_cs8
    complex double* vector_IQ = cargar_cs8(file_sample_str, &num_samples, &error_code);

    // Verificar si la carga falló
    if (vector_IQ == NULL) {
        // Obtener la descripción del error
        const char* error_msg = cs8_iq_error_string(error_code);
        fprintf(stderr, "Error al cargar los datos CS8: %s\n", error_msg);
        return;  // Salir de la función si falla la carga
    }

    // Si la carga es exitosa, continuar con el procesamiento
    printf("Total samples: %lu\n", num_samples);

    // Liberar recursos (asumiendo que delete_CS8 es parte del flujo original)
    delete_CS8(0);
    ...
    // Liberar la memoria asignada para vector_IQ
    free(vector_IQ);
